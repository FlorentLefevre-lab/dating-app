...
export function useEnhancedSocket(userId: string, userName?: string): UseEnhancedSocketReturn {
  ...
  const handleMessageReceived = useCallback((data: any) => {
    ...
    // Vérifier que ce message est pour la conversation actuelle
    const messageConversationId = data.conversationId || 
      `conv_${[data.senderId || data.from, data.receiverId || data.to].sort().join('_')}`;
    
    const newMessage: Message = {
      id: data.messageId || data.id,
      content: data.content,
      senderId: data.senderId || data.from,
      receiverId: data.receiverId || data.to,
      timestamp: data.timestamp,
      fromCurrentUser: (data.senderId || data.from) === userId,
      status: 'delivered',
      sender: data.sender
    };
  ...
  const syncOfflineMessages = useCallback(async () => {
    try {
      // Désactivé temporairement à cause de l'erreur IndexedDB
      console.log('🔄 Sync offline désactivée temporairement');
